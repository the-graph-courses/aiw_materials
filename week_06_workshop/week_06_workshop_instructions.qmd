...
1.  **Create a promotional flyer** for our course showcase using **Ideogram** for image generation and **Canva** for final editing. (You're encouraged to share this flyer with colleagues and friends to get them to come to the showcase.)
...
1.  Sign up for a free account on **ideogram.ai**. (We're using ideogram instead of ChatGPT's image generation feature because the free image generation on ChatGPT is quite slow and unreliable.)

2.  In the prompt box, describe the image you want to create. Here is the proposed prompt.


Ideogram prompt recommendation
...
1.  Go to **canva.com** Sign up and start a free trial for the pro features. Cancel the trial immediately after. (If you have already used up your free trial, please create a new account with a fresh email. If this is not possible, please contact an instructor for help.)
2.  Drag-and-drop your Ideogram image into the Canva interface
...
In this section, we'll use a design tool to turn a research paper into a visual abstract. A visual abstract is a pictorial representation of the main findings of a research paper, and is a great way to communicate the key results of a paper in an appealing way.
...
Sign up for a free account on Claude.ai and use Claude to create either an SVG or HTML-based "visual abstract" of the paper.

(We recommend Claude over ChatGPT for this task because a) it tends to be better at designing, and b) it gives you an easy way to download the SVG code.)

Give Claude one of the following prompts. Feel free to try both and compare the results.
...
After the initial prompt, you can iterate on the output with Claudeto refine the design.
...
4.  Ungroup/Convert to shape: 
- For those on Mac, right click on the image, click on Group, then Ungroup. You may need to do this a few times, until there is nothing left to ungroup.

![Ungroup in Powerpoint for Mac](images/ungroup_mac.jpg) 

- For those on Windows, right click on the image, click on Convert to shape.
...
5. When done, take a screenshot of your final result and upload it to the course page.
