...
The goal here is to build an extremely simple automation that demonstrates the power of connecting different web services with an AI Large Language Model (LLM).
...
2.  In the title field and enter: `Consultation Inquiry`
...
![](images/link_to_sheets.jpg){width="367"}
...
2.  Click the plus button to add a new module, and select **Google Sheets**

![](images/scenario_new_row.jpg){width="356"}

3.  Choose **Watch New Rows ACID**. This will watch for new rows in the spreadsheet, and trigger when someone fills out the connected form.

4.  Click **Creating a connection...** → **Sign in with Google**

5.  You may need to select your Google account and authorize access. Give the requested permissions.

6.  Now, configure the module settings:

    -   **Spreadsheet ID:** Navigate to and select your consultation form spreadsheet. It shoudl be called something like Consultation Inquiry.
...
2.  In the **Content** field, paste a prompt something like:

```         
A potential client has filled out a form on my website requesting generative AI consulting services. Research the company/organization below and create a clear, elegant markdown report that covers their an introduction to the business as well as potential generative AI opportunities for their work.
...
2.  Search for **Google Docs** and select **Create a new document**
...
Run your complete workflow test again and save your work. If successful, check your Google Drive for the newly created report document.
...
But connecting personal Gmail accounts to Make.com requires complex setup and many companies might require special permissions to create such a connection.
...
2.  On the homepage, create an API key and copy it
...
1.  Go to ChatGPT and open your existing portfolio conversation, or copy paste in the index.html code from your page.
2.  Provide a prompt something like this:
...
4.  Commit and push the changes to make your consultation form live
